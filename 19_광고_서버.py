...
from pydantic import BaseModel, Field # JSON 형태로 받는 거
...
mongo_client = MongoClient(DB_URL,serverSelectionTimeoutMS = 5000) # 5초 기다렸다가 연결 안 되면 에러

database = mongo_client['ai'] # AIdb
...
class AdGenerator:
    ...
    def using_llm(self, prompt: str) -> str:
        ...
        사용자가 제공한 제품 이름, 주요 내용, 광고 문구 스타일을 분석하여 자연스럽고 기억하기 쉬운 한국어 관고 문구를 작성하세요.
...
app = FastAPI(title= "AI 광고 문구 생성 API 이런거 안 넣어도 됨", description="GPT 모델과 MongoDB를 이용한 광고 문구 생성 API", version="1.0.0") 
# 이걸 app이라고 정의해서 uvicorn "19_광고_서버":app --reload 라고 써야 실행함  app 대신 q로 정의하면 uvicorn "19_광고_서버":q --reload 아마
...
def create_ad(product: ProductRequest):
    ...
    try:
        ...
        cursor = (collection.find({}, {"_id":0}).sort("_id", DESCENDING).limit(20)) # {} 조건 없이 id만 가져온다
